...
class Attention(nn.Module):
    ...
    def forward(self, x ,illumination_feature):
        b, c, h, w = x.shape
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x ,illumination_feature):
        x = x + self.attn(self.norm1(x),illumination_feature)
...
class Illumination_Estimator(nn.Module):
    ...
    def forward(self, img):
        ...
        mean_c = img.max(dim=1).unsqueeze(1)
        # stx()
...
class DRSformer(nn.Module):
    ...
    def forward(self, inp_img):
        inp_feature , _ = self.illumination_prior(inp_img)
        # print(type(inp_prior))
        # inp_enc_level1 = self.patch_embed(inp_prior)
        inp_enc_level1 = self.patch_embed(inp_img)
        inp_enc_level0 = self.encoder_level0(inp_enc_level1) ## We do not use MEFC for training Rain200L and SPA-Data
        # print(type(inp_enc_level0))
        # print(inp_enc_level0.shape)
        # print(type(inp_feature))
        # print(inp_feature.shape)
        ...
        # out_enc_level2 = self.encoder_level2(inp_enc_level2, inp_feature)
        x = inp_enc_level4
        for blk in self.latent:
            x = blk(x,inp_feature)
        latent = x
        # latent = self.latent(inp_enc_level4,inp_feature)
        ...
        for blk in self.decoder_level3:
            x=blk(x,inp_feature)
        out_dec_level3 = x
        # out_dec_level3 = self.decoder_level3(inp_dec_level3,inp_feature)
        ...
        for blk in self.decoder_level2:
            x=blk(x,inp_feature)   
        out_dec_level2 = x
        # out_dec_level2 = self.decoder_level2(inp_dec_level2,inp_feature)
        ...
        # out_dec_level1 = self.decoder_level1(inp_dec_level1,inp_feature)
